...
extends Carver {
                    ...
                    for (n4 = n5; !bl3 && n4 < n6; ++n4) {
                        for (int i = n9; !bl3 && i < n10; ++i) {
                            for (int j = n8 + 1; !bl3 && j >= n7 - 1; --j) {
                                ...
                                if (blocks[n3] == Block.FLOWING_WATER.id || blocks[n3] == Block.WATER.id) {
                                    bl3 = true;
                                }
                            ...
                            for (n3 = n9; n3 < n10; ++n3) {
                                ...
                                for (int i = n8 - 1; i >= n7; --i) {
                                    ...
                                    if (d11 > -0.7 && d9 * d9 + d11 * d11 + d10 * d10 < 1.0) {
                                        ...
                                        if (by == Block.STONE.id || by == Block.DIRT.id || by == Block.GRASS_BLOCK.id) {
                                            if (i < 10) {
                                                blocks[n11] = (byte)Block.FLOWING_LAVA.id;
                                            } else {
                                                blocks[n11] = 0;
                                                if (bl4 && blocks[n11 - 1] == Block.DIRT.id) {
                                                    blocks[n11 - 1] = (byte)Block.GRASS_BLOCK.id;
                                                }
                                            }
                                        }
                                    }
    ...
    protected void carve(World world, int startChunkX, int startChunkZ, int chunkX, int chunkZ, byte[] blocks) {
        int n = this.random.nextInt(this.random.nextInt(this.random.nextInt(40) + 1) + 1);
        if (this.random.nextInt(15) != 0) {
            n = 0;
        }
        for (int i = 0; i < n; ++i) {
            double d = startChunkX * 16 + this.random.nextInt(16);
            double d2 = this.random.nextInt(this.random.nextInt(120) + 8);
            ...
            for (int j = 0; j < n2; ++j) {
                ...
                this.carveTunnels(chunkX, chunkZ, blocks, d, d2, d3, f3, f, f2, 0, 0, 1.0);
            }
        }
    }
}
