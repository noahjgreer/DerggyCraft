...
public interface CollisionView extends BlockView {
	...
	default boolean isBlockSpaceEmpty(@Nullable Entity entity, Box box, boolean blockOrFluid) {
		for (VoxelShape voxelShape : blockOrFluid ? this.getBlockOrFluidCollisions(entity, box) : this.getBlockCollisions(entity, box)) {
	...
	default boolean doesNotCollideWithEntities(@Nullable Entity entity, Box box) {
		return this.getEntityCollisions(entity, box).isEmpty();
	}

	default boolean doesNotCollideWithWorldBorder(@Nullable Entity entity, Box box) {
		if (entity == null) {
			return true;
		} else {
