...
public class BookScreen extends Screen {
	...
	private static final Style STYLE = Style.EMPTY.withoutShadow().withColor(Colors.BLACK);
	...
	private Text getPageIndicatorText() {
		return Text.translatable("book.pageIndicator", this.pageIndex + 1, Math.max(this.getPageCount(), 1)).fillStyle(STYLE);
	}

	protected void addCloseButton() {
		this.addDrawableChild(
			ButtonWidget.builder(ScreenTexts.DONE, button -> this.close()).position((this.width - 200) / 2, this.getCloseButtonY()).width(200).build()
		);
	}
	...
	private void render(DrawnTextConsumer drawer, boolean click) {
		if (this.cachedPageIndex != this.pageIndex) {
			StringVisitable stringVisitable = Texts.withStyle(this.contents.getPage(this.pageIndex), STYLE);
