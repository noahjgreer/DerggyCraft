...
public class CreateWorldScreen extends Screen {
	...
	public static void show(MinecraftClient client, Runnable onClosed) {
		show(
			client, onClosed, (screen, combinedDynamicRegistries, levelProperties, dataPackTempDir) -> screen.startServer(combinedDynamicRegistries, levelProperties)
		);
	}
	...
	public static void show(MinecraftClient client, Runnable onClosed, CreateWorldCallback callback) {
		...
		show(client, onClosed, function, generatorOptionsFactory, WorldPresets.DEFAULT, callback);
	}
	...
	public static void showTestWorld(MinecraftClient client, Runnable onClosed) {
		GeneratorOptionsFactory generatorOptionsFactory = (dataPackContents, dynamicRegistries, settings) -> new GeneratorOptionsHolder(
			...
			new InitialWorldOptions(
				...
				new ServerGameRules.Builder().put(GameRules.ADVANCE_TIME, false).put(GameRules.ADVANCE_WEATHER, false).put(GameRules.DO_MOB_SPAWNING, false).build(),
	...
	private static void show(
		...
		client.setScreen(
			new CreateWorldScreen(client, onClosed, (GeneratorOptionsHolder)completableFuture.join(), Optional.of(presetKey), OptionalLong.empty(), callback)
		);
	}

	public static CreateWorldScreen create(
		MinecraftClient client, Runnable onClosed, LevelInfo levelInfo, GeneratorOptionsHolder generatorOptionsHolder, @Nullable Path dataPackTempDir
	) {
	...
	class GameTab extends GridScreenTab {
		...
		GameTab() {
			...
			CyclingButtonWidget<WorldCreator.Mode> cyclingButtonWidget = adder.add(
				CyclingButtonWidget.<WorldCreator.Mode>builder(value -> value.name, CreateWorldScreen.this.worldCreator.getGameMode())
			...
			CyclingButtonWidget<Difficulty> cyclingButtonWidget2 = adder.add(
				CyclingButtonWidget.builder(Difficulty::getTranslatableName, CreateWorldScreen.this.worldCreator.getDifficulty())
			...
			CreateWorldScreen.this.client
				.setScreen(
					new EditGameRulesScreen(
						CreateWorldScreen.this.worldCreator
							.getGameRules()
							.withEnabledFeatures(CreateWorldScreen.this.worldCreator.getGeneratorOptionsHolder().dataConfiguration().enabledFeatures()),
						gameRules -> {
	...
	class WorldTab extends GridScreenTab {
		...
		WorldTab() {
			...
			CyclingButtonWidget<WorldCreator.WorldType> cyclingButtonWidget = adder.add(
				CyclingButtonWidget.builder(WorldCreator.WorldType::getName, CreateWorldScreen.this.worldCreator.getWorldType())
