...
public class EditWorldScreen extends Screen {
	...
	private EditWorldScreen(MinecraftClient client, LevelStorage.Session session, String levelName, BooleanConsumer callback) {
		...
		this.layout.add(ButtonWidget.builder(RESET_ICON_TEXT, button -> {
			session.getIconFile().ifPresent(iconFile -> FileUtils.deleteQuietly(iconFile.toFile()));
			button.active = false;
		}).width(200).build()).active = session.getIconFile().filter(iconFile -> Files.isRegularFile(iconFile, new LinkOption[0])).isPresent();
		this.layout
