...
public class RealmsServer extends ValueObject implements RealmsSerializable {
	...
	public Map<Integer, RealmsSlot> cloneSlots(Map<Integer, RealmsSlot> slots) {
		...
		for (Entry<Integer, RealmsSlot> entry : slots.entrySet()) {
			map.put(
				(Integer)entry.getKey(), new RealmsSlot((Integer)entry.getKey(), ((RealmsSlot)entry.getValue()).options.copy(), ((RealmsSlot)entry.getValue()).settings)
			);
		}
	...
	public static enum WorldType {
		...
		private WorldType(final String id) {
			this.text = Text.translatable("mco.backup.entry.worldType." + id);
		}
