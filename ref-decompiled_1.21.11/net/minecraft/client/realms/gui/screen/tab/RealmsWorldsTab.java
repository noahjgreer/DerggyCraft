...
class RealmsWorldsTab extends GridScreenTab implements RealmsUpdatableTab {
	...
	RealmsWorldsTab(RealmsConfigureWorldScreen screen, MinecraftClient client, RealmsServer server) {
		...
		this.optionsButton = adder3.add(
			ButtonWidget.builder(
					...
					button -> client.setScreen(
						new RealmsSlotOptionsScreen(screen, ((RealmsSlot)server.slots.get(server.activeSlot)).copy(), server.worldType, server.activeSlot)
					)
				)
		...
		this.backupButton = adder3.add(
			ButtonWidget.builder(
					Text.translatable("mco.configure.world.backup"), button -> client.setScreen(new RealmsBackupScreen(screen, server.copy(), server.activeSlot))
				)
				.dimensions(0, 0, 150, 20)
				.build()
		);
	...
	private void reset() {
		if (this.isMinigame()) {
			...
			this.client
				.setScreen(
					RealmsCreateWorldScreen.resetWorld(this.screen, this.server.copy(), () -> this.client.execute(() -> this.client.setScreen(this.screen.getNewScreen())))
				);
		}
	}
