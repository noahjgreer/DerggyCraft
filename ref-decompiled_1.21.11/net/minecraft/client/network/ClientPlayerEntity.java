...
public class ClientPlayerEntity extends AbstractClientPlayerEntity {
		...
		if (!super.startRiding(entity, force, emitEvent)) {
			...
			if (entity instanceof AbstractMinecartEntity abstractMinecartEntity) {
				...
				this.client
					.getSoundManager()
					.play(
						new EntityRidingSoundInstance(this, happyGhastEntity, false, SoundEvents.ENTITY_HAPPY_GHAST_RIDING, happyGhastEntity.getSoundCategory(), 0.0F, 1.0F, 5.0F)
					);
			} else if (entity instanceof AbstractNautilusEntity abstractNautilusEntity) {
				this.client
					.getSoundManager()
					.play(
						new EntityRidingSoundInstance(
	...
	public HitResult getCrosshairTarget(float tickProgress, Entity cameraEntity) {
		...
		if (attackRangeComponent != null) {
			hitResult = attackRangeComponent.getHitResult(cameraEntity, tickProgress, EntityPredicates.CAN_HIT);
			if (hitResult instanceof BlockHitResult) {
				hitResult = checkCrosshairTargetRange(hitResult, cameraEntity.getCameraPosVec(tickProgress), d);
			}
		}

		if (hitResult == null || hitResult.getType() == HitResult.Type.MISS) {
			double e = this.getEntityInteractionRange();
			hitResult = getCrosshairTarget(cameraEntity, d, e, tickProgress);
		}
	...
	private static HitResult getCrosshairTarget(Entity cameraEntity, double blockInteractionRange, double entityInteractionRange, float tickProgress) {
		...
		return entityHitResult != null && entityHitResult.getPos().squaredDistanceTo(vec3d) < f
			? checkCrosshairTargetRange(entityHitResult, vec3d, entityInteractionRange)
			: checkCrosshairTargetRange(hitResult, vec3d, blockInteractionRange);
	}
