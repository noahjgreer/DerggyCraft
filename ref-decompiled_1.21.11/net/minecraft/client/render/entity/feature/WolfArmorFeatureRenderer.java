...
public class WolfArmorFeatureRenderer extends FeatureRenderer<WolfEntityRenderState, WolfEntityModel> {
	...
	private void renderCracks(
		MatrixStack matrices, OrderedRenderCommandQueue queue, int light, ItemStack stack, Model<WolfEntityRenderState> model, WolfEntityRenderState state
	) {
		...
		if (crackLevel != Cracks.CrackLevel.NONE) {
			...
			queue.submitModel(model, state, matrices, RenderLayers.armorTranslucent(identifier), light, OverlayTexture.DEFAULT_UV, state.outlineColor, null);
		}
	}
}
