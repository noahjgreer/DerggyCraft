...
public class LlamaDecorFeatureRenderer extends FeatureRenderer<LlamaEntityRenderState, LlamaEntityModel> {
	...
	private void render(
		MatrixStack matrices, OrderedRenderCommandQueue queue, LlamaEntityRenderState state, ItemStack stack, RegistryKey<EquipmentAsset> assetKey, int light
	) {
		LlamaEntityModel llamaEntityModel = state.baby ? this.babyModel : this.model;
		this.equipmentRenderer.render(EquipmentModel.LayerType.LLAMA_BODY, assetKey, llamaEntityModel, state, stack, matrices, queue, light, state.outlineColor);
	}
}
