	...
	public WardenEntityRenderer(EntityRendererFactory.Context context) {
		...
		this.addFeature(
			new EmissiveFeatureRenderer<>(
				this, state -> BIOLUMINESCENT_LAYER_TEXTURE, (state, tickProgress) -> 1.0F, wardenEntityModel, RenderLayers::entityTranslucentEmissive, false
			)
		);
		this.addFeature(
			new EmissiveFeatureRenderer<>(
				this,
				state -> PULSATING_SPOTS_1_TEXTURE,
		...
		this.addFeature(
			new EmissiveFeatureRenderer<>(
				this,
				state -> PULSATING_SPOTS_2_TEXTURE,
		...
		this.addFeature(
			new EmissiveFeatureRenderer<>(
				this, state -> TEXTURE, (state, tickProgress) -> state.tendrilAlpha, wardenEntityModel3, RenderLayers::entityTranslucentEmissive, false
			)
		);
		this.addFeature(
			new EmissiveFeatureRenderer<>(
				this, state -> HEART_TEXTURE, (state, tickProgress) -> state.heartAlpha, wardenEntityModel4, RenderLayers::entityTranslucentEmissive, false
			)
		);
	}
